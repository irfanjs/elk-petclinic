...
public class SimpleJdbcClinic implements Clinic, SimpleJdbcClinicMBean {
    ...
    public void init(DataSource dataSource) {
        ...
        this.insertFeedback = new SimpleJdbcInsert(dataSource)
    ...
    @Transactional(readOnly = true)
    public Collection<Feedback> findFeedbacks(String lastName) throws DataAccessException {
        List<Feedback> feedbacks = this.simpleJdbcTemplate.query(
                "SELECT id, first_name, last_name, address, city, telephone, details FROM feedbacks WHERE last_name like ?",
                ParameterizedBeanPropertyRowMapper.newInstance(Feedback.class),
                lastName + "%");
        loadFeedbacksPetsAndVisits(feedbacks);
        return feedbacks;
    }
    ...
    @Transactional(readOnly = true)
    public Feedback loadFeedback(int id) throws DataAccessException {
        Feedback feedback;
        try {
            feedback = this.simpleJdbcTemplate.queryForObject(
                    "SELECT id, first_name, last_name, address, city, telephone, details FROM feedbacks WHERE id=?",
                    ParameterizedBeanPropertyRowMapper.newInstance(Feedback.class),
                    id);
        }
        catch (EmptyResultDataAccessException ex) {
            throw new ObjectRetrievalFailureException(Feedback.class, id, "", ex);
        }
        loadPetsAndVisits1(feedback);
        return feedback;
    }
	...
	public void storeFeedback(Feedback feed) throws DataAccessException {
		 if (feed.isNew()) {
			 logger.info("in if loop of storefeedback" );
              Number newKey = this.insertFeedback.executeAndReturnKey(
                      new BeanPropertySqlParameterSource(feed));
              feed.setId(newKey.intValue());
          }
          else {
        	  logger.info("in else loop of storefeedback" );
              this.simpleJdbcTemplate.update(
                      "UPDATE feedbacks SET first_name=:firstName, last_name=:lastName, address=:address, " +
                      "city=:city, telephone=:telephone, details=:details WHERE id=:id",
                      new BeanPropertySqlParameterSource(feed));
          }
    ...
    private void loadPetsAndVisits1(final Feedback feedback) {
        final List<JdbcPet> pets = this.simpleJdbcTemplate.query(
                "SELECT id, name, birth_date, type_id, owner_id FROM pets WHERE owner_id=?",
                new JdbcPetRowMapper(),
                feedback.getId().intValue());
        for (JdbcPet pet : pets) {
            feedback.addPet(pet);
            pet.setType(EntityUtils.getById(getPetTypes(), PetType.class, pet.getTypeId()));
            loadVisits(pet);
    ...
    private void loadFeedbacksPetsAndVisits(List<Feedback> feedbacks) {
